...
$cat_opts = '<option value="">Select a Category</option>'."\n";
for ($i = 0, $iz = count($category->category); $i < $iz; ++$i) {
   $cat = &$category->category[$i];
   $cat_opts .= '<optgroup label="'.$cat['parent']['title'].'">'."\n";
   $cat_opts .= '  <option value="'.$cat['parent']['id'].'">'.$cat['parent']['title'].' > All</option>'."\n";

   for ($j = 0, $jz = count($cat['children']); $j < $jz; ++$j) {
      $child = &$cat['children'][$j];
      $cat_opts .= '  <option value="'.$child['id'].'">'.$cat['parent']['title'].' &gt; '.$child['title'].'</option>'."\n";
   }
